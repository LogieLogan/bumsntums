    ...
    try {
      _analytics.logEvent(
        ...
        parameters: {
          ...
          'hasTextRequest':
              additionalParams?.containsKey('textRequest') ?? false,
        },
      );
      ...
      final plan = await _planningRepository.createWorkoutPlan(
        ...
        description:
            additionalParams?.containsKey('textRequest') ?? false
                ? 'Generated by AI based on request: ${additionalParams!['textRequest']}'
                : 'Generated by AI based on your goals and preferences',
      );

      // Get suitable workouts with validation
      ...
      for (final area in focusAreas) {
        ...
        if (category != null) {
          ...
          try {
            final categoryWorkouts = await _workoutService
            ...
            // Validate workouts to ensure exercises are valid
            for (final workout in filteredWorkouts) {
              // Check that all exercises in this workout have valid references
              final hasValidExercises = workout.exercises.every(
                (exercise) =>
                    exercise.id.isNotEmpty &&
                    exercise.name.isNotEmpty &&
                    !exercise.name.contains("Not Found"),
              );

              if (hasValidExercises) {
                workouts.add(workout);
              } else {
                print('Skipping workout with invalid exercises: ${workout.id}');
              }
            }
          } catch (e) {
